...
def saveToFile():
        ...
        print "Triggered, saving image to file"
        # write picture to temp file
        t = TempImage()
        ...
        destFile= "{dest}/{timestamp}.jpg".format(dest=conf["folder_path"], timestamp=ts)
	try:
	        print "Copying to " + destFile
	        shutil.copyfile(t.path, destFile)
        	# Clean up
	        os.remove(t.path)
        except:
...
def saveToVideo():
	...
	print "Saving to Temp file" + t.path
        camera.start_recording(t.path, quality=20, bitrate=500000)            
